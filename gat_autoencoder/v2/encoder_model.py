...
class MyGATv2EncoderVAE(torch.nn.Module):
    def __init__(self):
        super(MyGATv2EncoderVAE, self).__init__()

        # First GATv2 layer
        self.conv1 = GATv2Conv(in_channels=29, out_channels=16, heads=3, concat=True, dropout=0.3)
        self.bn1 = BatchNorm1d(16 * 3)

        # Second GATv2 layer with residual connection
        self.conv2 = GATv2Conv(in_channels=16 * 3, out_channels=16, heads=3, concat=True, dropout=0.3)
        self.bn2 = BatchNorm1d(16 * 3)

        # Third GATv2 layer with residual connection
        self.conv3 = GATv2Conv(in_channels=16 * 3, out_channels=16, heads=3, concat=True, dropout=0.3)
        self.bn3 = BatchNorm1d(16 * 3)

        # Fourth GATv2 layer
        self.conv4 = GATv2Conv(in_channels=16 * 3, out_channels=8, heads=5, concat=False, dropout=0.3)
        self.bn4 = BatchNorm1d(8)

        # Output layers for mean and log-variance of the latent space
        self.fc_mu = torch.nn.Linear(8, 8)  # Mean of the latent space
        self.fc_log_var = torch.nn.Linear(8, 8)  # Log variance of the latent space
